import { Router } from 'express';
import RouteController from '../controllers/route.controller.js';
import RouteService from '../services/route.service.js';

const router = Router();

// 서비스와 컨트롤러 인스턴스 생성
const routeService = new RouteService();
const routeController = new RouteController(routeService);

/**
 * @swagger
 * /route/walking:
 *   post:
 *     summary: 도보 길안내
 *     description: |
 *       출발지에서 목적지까지의 도보 경로를 안내합니다.
 *       예상 소요 시간과 거리 정보를 포함합니다.
 *     tags:
 *       - 길안내
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
 *           schema:
 *             type: object
 *             required:
 *               - startLat
 *               - startLng
 *               - endLat
 *               - endLng
 *             properties:
 *               startLat:
 *                 type: number
 *                 format: double
 *                 description: 출발지 위도
 *                 example: 37.5665
 *               startLng:
 *                 type: number
 *                 format: double
 *                 description: 출발지 경도
 *                 example: 126.9780
 *               endLat:
 *                 type: number
 *                 format: double
 *                 description: 목적지 위도
 *                 example: 37.5700
 *               endLng:
 *                 type: number
 *                 format: double
 *                 description: 목적지 경도
 *                 example: 126.9800
 *     responses:
 *       200:
 *         description: 도보 경로 안내 성공
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 *               properties:
 *                 success:
 *                   type: boolean
 *                   example: true
 *                 code:
 *                   type: string
 *                   example: "ROUTE-200-001"
 *                 message:
 *                   type: string
 *                   example: "도보 경로 조회 성공"
 *                 data:
 *                   type: object
 *                   properties:
 *                     route:
 *                       type: object
 *                       properties:
 *                         start:
 *                           type: object
 *                           properties:
 *                             lat:
 *                               type: number
 *                               example: 37.5665
 *                             lng:
 *                               type: number
 *                               example: 126.9780
 *                         end:
 *                           type: object
 *                           properties:
 *                             lat:
 *                               type: number
 *                               example: 37.5700
 *                             lng:
 *                               type: number
 *                               example: 126.9800
 *                         distance:
 *                           type: number
 *                           description: 거리 (미터)
 *                           example: 450
 *                         estimatedDuration:
 *                           type: number
 *                           description: 예상 소요 시간 (분)
 *                           example: 7
 *                         transportType:
 *                           type: string
 *                           example: "WALKING"
 *                     navigation:
 *                       type: object
 *                       properties:
 *                         available:
 *                           type: boolean
 *                           example: false
 *                         message:
 *                           type: string
 *                           example: "세부 경로는 카카오맵에서 확인하세요"
 *                         kakaoMapUrl:
 *                           type: string
 *                           example: "https://map.kakao.com/link/to/목적지,37.5700,126.9800"
 *                 timestamp:
 *                   type: string
 *                   format: date-time
 *                   example: "2026-01-31T12:00:00.000Z"
 *       400:
 *         description: 필수 파라미터 누락 또는 유효하지 않은 좌표
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 *               properties:
 *                 success:
 *                   type: boolean
 *                   example: false
 *                 code:
 *                   type: string
 *                   example: "COM-400-001"
 *                 message:
 *                   type: string
 *                   example: "출발지와 목적지 좌표는 필수입니다"
 *                 path:
 *                   type: string
 *                   example: "/route/walking"
 *                 timestamp:
 *                   type: string
 *                   format: date-time
 *       500:
 *         description: 서버 오류
 */
router.post('/walking', (req, res, next) => 
  routeController.getWalkingRoute(req, res, next)
);

/**
 * @swagger
 * /route/navigation:
 *   post:
 *     summary: 길찾기 경로 안내
 *     description: |
 *       대중교통 또는 자동차 경로를 안내합니다.
 *       교통수단별 예상 소요 시간, 요금, 경유지 정보를 제공합니다.
 *     tags:
 *       - 길안내
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
 *           schema:
 *             type: object
 *             required:
 *               - startLat
 *               - startLng
 *               - endLat
 *               - endLng
 *               - transportType
 *             properties:
 *               startLat:
 *                 type: number
 *                 format: double
 *                 description: 출발지 위도
 *                 example: 37.5665
 *               startLng:
 *                 type: number
 *                 format: double
 *                 description: 출발지 경도
 *                 example: 126.9780
 *               endLat:
 *                 type: number
 *                 format: double
 *                 description: 목적지 위도
 *                 example: 37.5700
 *               endLng:
 *                 type: number
 *                 format: double
 *                 description: 목적지 경도
 *                 example: 126.9800
 *               transportType:
 *                 type: string
 *                 enum: [PUBLIC, CAR]
 *                 description: 교통수단 타입 (PUBLIC-대중교통, CAR-자동차)
 *                 example: "PUBLIC"
 *     responses:
 *       200:
 *         description: 경로 안내 성공
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 *               properties:
 *                 success:
 *                   type: boolean
 *                   example: true
 *                 code:
 *                   type: string
 *                   example: "ROUTE-200-002"
 *                 message:
 *                   type: string
 *                   example: "PUBLIC 경로 조회 성공"
 *                 data:
 *                   type: object
 *                   properties:
 *                     route:
 *                       type: object
 *                       properties:
 *                         start:
 *                           type: object
 *                           properties:
 *                             lat:
 *                               type: number
 *                               example: 37.5665
 *                             lng:
 *                               type: number
 *                               example: 126.9780
 *                         end:
 *                           type: object
 *                           properties:
 *                             lat:
 *                               type: number
 *                               example: 37.5700
 *                             lng:
 *                               type: number
 *                               example: 126.9800
 *                         distance:
 *                           type: number
 *                           description: 거리 (미터)
 *                           example: 5000
 *                         estimatedDuration:
 *                           type: number
 *                           description: 예상 소요 시간 (분)
 *                           example: 15
 *                         transportType:
 *                           type: string
 *                           enum: [PUBLIC, CAR]
 *                           example: "PUBLIC"
 *                         estimatedFare:
 *                           type: number
 *                           description: 예상 요금 (원, 대중교통만 해당)
 *                           example: 1400
 *                           nullable: true
 *                     navigation:
 *                       type: object
 *                       properties:
 *                         available:
 *                           type: boolean
 *                           example: false
 *                         message:
 *                           type: string
 *                           example: "대중교통 경로는 카카오맵에서 확인하세요"
 *                         kakaoMapUrl:
 *                           type: string
 *                           example: "https://map.kakao.com/link/to/목적지,37.5700,126.9800"
 *                 timestamp:
 *                   type: string
 *                   format: date-time
 *                   example: "2026-01-31T12:00:00.000Z"
 *       400:
 *         description: 필수 파라미터 누락 또는 잘못된 교통수단 타입
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 *               properties:
 *                 success:
 *                   type: boolean
 *                   example: false
 *                 code:
 *                   type: string
 *                   example: "COM-400-001"
 *                 message:
 *                   type: string
 *                   example: "유효하지 않은 교통수단 타입입니다"
 *                 path:
 *                   type: string
 *                   example: "/route/navigation"
 *                 details:
 *                   type: object
 *                   properties:
 *                     validTypes:
 *                       type: array
 *                       items:
 *                         type: string
 *                       example: ["PUBLIC", "CAR"]
 *                     receivedType:
 *                       type: string
 *                       example: "INVALID"
 *                 timestamp:
 *                   type: string
 *                   format: date-time
 *       500:
 *         description: 서버 오류
 */
router.post('/navigation', (req, res, next) => 
  routeController.getNavigation(req, res, next)
);

export default router;